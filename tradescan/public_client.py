...
class PublicClient(object):
    ...
    def get_block(self, block_nbr = 0):
        if block_nbr is not None and isinstance(block_nbr, int) and block_nbr > 0:
            return self.request.get(path = '/blocks/' + str(block_nbr))
    ...
    def get_commitment_curve(self):
        return self.request.get(path = '/get_commitment_curve')

    def get_distribution_parameters(self):
        return self.request.get(path = '/distribution/parameters')
    ...
    def get_inflation_start_time(self):
        return self.request.get(path = '/get_inflation_start_time')

    def get_latest_blocks(self):
        return self.request.get(path = '/blocks/latest')

    def get_liquidity_pools(self):
        return self.request.get(path = '/get_liquidity_pools')

    def get_liquidations(self):
        return self.request.get(path = '/get_liquidations')
    ...
    def get_reward_curve(self):
        return self.request.get(path = '/get_reward_curve')
    ...
    def get_transaction_fees(self):
        return self.request.get(path = '/get_txns_fees')

    def get_transaction_log(self, transaction_hash = None):
        api_params = {}
        if transaction_hash is not None and isinstance(transaction_hash, str):
            api_params["hash"] = transaction_hash
        return self.request.get(path = '/get_tx_log', params = api_params)
    ...
    def get_validator_delegations(self, operator_address):
        return self.request.get(path = '/staking/validators/' + operator_address + '/delegations')
    ...
    def get_validator_signing_info(self, limit = 100):
        ...
        if limit is not None and isinstance(limit, int) and limit > 0:
            api_params["limit"] = limit
        return self.request.get(path = '/slashing/signing_infos', params = api_params)

    def get_validators(self, status = None):
        api_params = {}
        if status is not None and status in ["unbonding", "unbonded"]:
            api_params["status"] = status
        return self.request.get(path = '/staking/validators', params = api_params)
